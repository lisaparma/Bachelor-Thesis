...
Questo IDE non è stato imposto dall'azienda ma scelto personalmente per lo sviluppo dell'applicazione in quanto avevo già dimestichezza con esso ed è stato consigliato dai dipendenti che sviluppano con JavaScript.
...
dispositivi mobili.
L'utilizzo di questo tool è stato un requisito obbligatorio per il progetto. La validazione di Expo è infatti uno degli scopi dello stage in quanto l'azienda vorrebbe implementarla nei suoi progetti futuri.
...
In parallelo con TypeScript l'utilizzo di TSLint non è stato obbligatorio ma dato che il linguaggio per me era nuovo ho preferito utilizzare questo linter per essere più precisa nella grammatica del codice.
...
Nella zimlet OpenChat era già stata utilizzata questa tecnologia e volendo sincronizzare i dati e mantenere la struttura il più simile possibile si è voluto continuare ad usare questa soluzione anche perchè è l'unico modo per gestire in maniera semplice un numero di dati ampio e in continuo cambiamento.
...
\end{itemize}
Purtroppo la scelta di utilizzare questo debugger è stata fatta in base al fatto che è l'unica soluzione che include tutte queste funzionalità per React Native.
