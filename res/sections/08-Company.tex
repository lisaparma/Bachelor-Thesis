...
\begin{figure}[H] 
	\centering
	\includegraphics[scale=0.3]{loghi/zimbra}
	\caption{Logo Zimbra}
...
\emph{Bamboo} è un \emph{server} sviluppato da \emph{Atlassian} per la \gl{continuous integration}{continuous integration} e \gl{deployment}{deployment}. Tra le varie funzionalità, permette di eseguire più \emph{build} in parallelo, per un completamento più rapido, e in caso di errori di compilazione fornisce un'analisi di quest'ultimi, inclusa la \gl{stack trace}{stack trace}.
